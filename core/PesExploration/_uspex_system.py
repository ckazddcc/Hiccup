...
class UspexSystem:
    ...
    def write_poscars(atoms_list, output_file):
        ...
        for i, atoms in enumerate(atoms_list):
            ...
            write(tmp_file, atoms, direct=True, vasp5=True)
            ca = atoms.get_cell_lengths_and_angles()
            with open(tmp_file, "r") as f:
                content = f.readlines()
            content[0] = "EA%d  %.3f %.3f %.3f %.2f %.2f %.2f Sym.group:    0\n" % (
            (i + 1), ca[0], ca[1], ca[2], ca[3], ca[4], ca[5])
            # content[0] = f"EA{i + 1}\n"
...
def write_to_db(work_dir, constraint_z=0.0):
    """
...
def run_uspexs(elements,
    ...
    if connect(random_seeds_path).count() > 0 and dimension == 2:
        # 删除cell大于substrate的1.4*cell的结构
        sub = read(substrate_path)
        cell = sub.get_cell()
        cell_x = cell[0][0] * 1.4
        cell_y = cell[1][1] * 1.4
        cell_z = cell[2][2] + 14.5
        for row in connect(random_seeds_path).select():
            atoms = row.toatoms()
            seeds_cell = atoms.get_cell()
            if seeds_cell[0][0] > cell_x or seeds_cell[1][1] > cell_y or seeds_cell[2][2] >= cell_z:
                connect(random_seeds_path).delete([row.id])
        print("Valid random seeds: ", connect(random_seeds_path).count())

    # set_start_method('spawn', force=True)
    # 生成uspex的输入文件
    uspex_systems = []
    for i, target_composition in enumerate(compositions):
        if len(init_seeds_paths) == len(target_composition):
            init_seeds_path = init_seeds_paths[1]
        else:
            init_seeds_path = init_seeds_paths[0]
        uspex = UspexSystem(
            elements=elements,
            target_composition=target_composition,
            dimension=dimension,
            workdir=workdir,
            uspex_templates_dir=uspex_templates_dir,
            generation_num=generation_num,
            pop_size=pop_size,
            ini_pop_size=ini_pop_size,
            substrate_path=substrate_path,
            random_seeds_path=random_seeds_path,
            init_seeds_path=init_seeds_path,
        ...
        uspex_systems.append(uspex)

    # 提交uspex
    compos_dirs = []
    for uspex in uspex_systems:
        compos_dir = os.path.join(workdir, uspex.unique_mark)
        compos_dirs.append(compos_dir)
...
def merge_dbs(dbs_list, merge_db):
    merge = connect(merge_db)
    for db_path in dbs_list:
        db = connect(db_path)
        for row in db.select():
            atoms = row.toatoms()
            data = row.data
            kvp = row.key_value_pairs
            merge.write(atoms, data=data, key_value_pairs=kvp)
    print(f"DBs merged into {merge_db}: {merge.count()} successfully.")


if __name__ == '__main__':
    run_uspexs(elements=["O", "Cu", "Y"],
               compositions=[[1, 72, 8]],
               dimension=2,
               workdir="/home/cchen/CuY/gcga/r1/ga0",
               uspex_templates_dir="/home/cchen/CuY/gcga/template/uspex",
               generation_num=10,
               pop_size=20,
               ini_pop_size=20,
               substrate_path="/home/cchen/CuY/gcga/POSCAR_SUBSTRATE",
               random_seeds_path="/home/cchen/CuY/gcga/seeds_opt.db",
               init_seeds_paths=["/home/cchen/CuY/gcga/seeds_opt.db"],
               # init_seeds_paths=["/home/cchen/CuY/round1/Cu40Y8/ga/2_O1Cu62Y9/results1/goodStructures_POSCARS",
               #                   "/home/cchen/CuY/round1/Cu40Y8/ga/2_O2Cu52Y9/results1/goodStructures_POSCARS",
               #                   "/home/cchen/CuY/round1/Cu40Y8/ga/2_O3Cu50Y10/results1/goodStructures_POSCARS",
               #                   "/home/cchen/CuY/round1/Cu40Y8/ga/2_O3Cu52Y10/results1/goodStructures_POSCARS"],
               gpus=[3],
               uspex_env="/home/cchen/.conda/envs/uspex/bin",
               model_path="/home/cchen/CuY/gcga/frozen_model.pb",
               calculator="DP",
               nsw=100,
               ediffg=0.1,
               opt_method="BFGS"
               )
    # write_to_db("/home/cchen/CuY/round1/Cu48/ga1/2_O3Cu78Y2", constraint_z=3)
    # merge_dbs([
    #            "/home/cchen/CuY/hiccup/hiccup2/dp/nn1/dbs/iter.db",
    #            "/home/cchen/CuY/hiccup/hiccup2/pes/ga/ga1/sp_2.db",
    #            "/home/cchen/CuY/hiccup/hiccup2/pes/ga/ga1/sp_1.db",
    #            "/home/cchen/CuY/hiccup/hiccup2/pes/ga/ga1/opt.db",
    #            "/home/cchen/CuY/hiccup/hiccup2/pes/ga/ga1/md_traj.db"
    # ],
    #           "/home/cchen/CuY/hiccup/init3.db")
